...
if (!content.includes("async function sendDailyReport()")) {
  ...
  setTimeout(() => {
    console.log("Triggering test report...");
    sendDailyReport();
  }, 5000);
`;

    // Replace the old cron block
    content = content.replace(/(\/\/ Daily Telegram Report \(23:59\))[\s\S]*?(cron\.schedule\('59 23 \* \* \*', async \(\) => \{[\s\S]*?\}\);)/, reportFunction);
